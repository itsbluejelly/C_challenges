...
long long int _toDigits_Long_Long(long long int value, int digits)
{
    ...
    // DECLARING VARIABLES
    char *string_number = _toString_Long_Long(value, 10);
    int power = pow(10, strlen(string_number) - digits);
    // ROUNDING OFF THE NUMBER AS EXPECTED
    long long int rounded_number = round((double)value / power) * power;

    return rounded_number;
}

// THE LONG_Long_INT STRUCT
Long_Long_Int_type Long_Long_Int = {
    .toString = _toString_Long_Long,
    .toDigits = _toDigits_Long_Long
};