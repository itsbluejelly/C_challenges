...
#include <stdlib.h>
#include <stdbool.h>
// IMPORTING UNSIGNED_LONG_INT TYPE
#include "Unsigned_Long_Int.h"
#include "../String/ModifiedString.c"

// DECLARING THE TOSTRING FUNCTION
char *_toString_Unsigned_Long(unsigned long int value, int radix){
...
unsigned long int _toDigits_Unsigned_Long(unsigned long int value, int digits)
{
    ...
    // DECLARING VARIABLES
    char *string_number = _toString_Unsigned_Long(value, 10);
    int power = pow(10, strlen(string_number) - digits);
    // ROUNDING OFF THE NUMBER AS EXPECTED
    unsigned long int rounded_number = round((double)value / power) * power;

    return rounded_number;
}

// THE UNSIGNED_Long_INT STRUCT
Unsigned_Long_Int_type Unsigned_Long_Int = {
    .toString = _toString_Unsigned_Long,
    .toDigits = _toDigits_Unsigned_Long
};